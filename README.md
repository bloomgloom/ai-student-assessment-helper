...
학생 평가, 채점, 세특 기록 작성을 보조하는 로컬 웹 애플리케이션입니다. 나이스에서 내려받은 엑셀 파일을 업로드해 성취/평가기준, 수행평가 영역, 학생별 채점 기록, 세특 초안을 한곳에서 관리할 수 있습니다.
...
```bash
git clone https://github.com/p4r4d3/ai-student-assessment-helper.git
...
`기준 관리` 메뉴에서 성취기준/평가기준 파일을 업로드합니다.

나이스에서 성취기준 및 성취수준 파일을 엑셀로 저장한 뒤 업로드합니다. 업로드가 끝나면 과목과 영역별 성취/평가기준이 왼쪽 목록에 표시됩니다.

### 5-3. 영역 관리

`영역 관리` 메뉴에서 영역 관리 엑셀 파일을 업로드합니다.
...
영역 관리가 먼저 되어 있어야 기록 관리에서 채점 파일과 학생 기록을 영역별로 다루기 쉽습니다.

### 5-4. 기록 관리

`기록 관리` 메뉴에서 채점 파일과 과목세특 파일을 업로드합니다.
...
업로드 후 반, 과목, 영역을 선택해 학생별 기록을 확인하고 수정합니다. 환경 설정이 완료되어 있으면 AI 생성 기능으로 채점 기록이나 세특 초안을 만들 수 있습니다.

### 5-5. 산출물 업로드와 내보내기

기록 관리 화면에서 학생별 또는 영역별 산출물을 업로드할 수 있습니다. ZIP 파일을 사용하면 여러 파일을 한 번에 올릴 수 있습니다.
