...
처음 사용하는 경우에는 Docker 실행을 권장합니다. 코드를 수정하거나 업데이트를 자주 받을 경우에는 개발/수정용 실행을 사용하세요.

## 1. 쉬운 실행: Docker 사용

Node.js, npm, Git을 따로 설치하지 않아도 됩니다.
...
1. GitHub 저장소 페이지에서 초록색 `Code` 버튼을 누릅니다.
2. `Download ZIP`을 클릭합니다.
3. 내려받은 ZIP 파일의 압축을 풉니다.
4. 터미널에서 압축을 푼 프로젝트 폴더로 이동합니다.
...
업로드 파일과 데이터베이스는 프로젝트 폴더의 `storage/`에 저장됩니다.

## 2. 개발/수정용 실행: Git + Node.js/npm 사용

코드를 수정하거나 Git으로 업데이트를 받을 경우 이 방법을 사용합니다.
...
- [Node.js 20.19 이상](https://nodejs.org/en/download)
- npm: Node.js를 설치하면 함께 설치됩니다.
...
나이스 수행평가에 반영되지 않는 활동도 세특 작성을 위한 참고 영역으로 추가할 수 있습니다. 이 영역은 채점 파일의 수행평가 점수 입력 대상은 아니지만, 기록 기준과 세특 기준을 정리해 AI 기록 생성에 활용할 수 있습니다.

채점 파일은 평가 영역 관리가 완성되지 않아도 업로드할 수 있습니다. 다만 AI 채점과 기록 생성은 평가 영역 관리에서 정리한 수행평가 영역, 채점 기준, 기록 기준을 기준으로 동작합니다. 자동화를 정확히 사용하려면 평가 영역 관리에서 영역과 기준을 먼저 정리하세요.
...
- 파일명에 `수행평가 파일일괄등록`이 포함되면 채점 파일로 처리됩니다.
- 위 두 문구가 모두 없는 파일은 업로드할 수 없습니다.

나이스에서 내려받은 채점 파일과 과목세특 파일은 파일명을 그대로 유지하는 것을 권장합니다. 앱은 파일명에서 학년도, 학기, 학년, 과목, 강의실 정보를 읽어 같은 강의실 데이터를 연결합니다.
...
평가 영역 관리와 채점 파일의 영역이 다르면 업로드 결과에 `영역 불일치` 경고가 표시됩니다. 이 경우 업로드는 진행되지만, 앱의 자동화 기준은 평가 영역 관리의 `반영 여부 = O` 영역을 우선합니다. 경고가 표시되면 평가 영역 관리와 채점 파일의 영역명, 개수, 만점을 확인하세요.
...
파일을 업로드한 뒤에는 평가 영역 관리에서 각 영역의 기준을 먼저 정리합니다. 기준이 정리되지 않은 상태에서도 표를 직접 편집하고 내보낼 수 있지만, AI 자동화는 기준이 준비된 영역에서 가장 안정적으로 동작합니다.
...
ZIP으로 산출물을 일괄 업로드할 때 각 파일명에는 `5자리 학번 + 이름`이 포함되어야 합니다. 학번은 이름보다 앞에 있어야 하며, 앱의 학생 명단에 있는 학번과 이름이 모두 일치해야 해당 학생에게 연결됩니다.

예:

- `20301홍길동.py`
- `20301_홍길동_보고서.hwpx`
- `수행평가1_20301_홍길동.ipynb`
...
### Docker 실행 후 접속이 안 되는 경우

Docker Desktop이 실행 중인지 확인한 뒤 프로젝트 폴더에서 다시 실행하세요.
...
- 성취 기준 관리 개선
  - 현재는 파일 업로드 또는 과목 이름 선택 방식으로 성취 기준을 관리함
  - 업로드/선택된 성취 기준을 화면에서 직접 편집할 수 있도록 기능 추가
- 학생 접속 페이지 추가
  - 수행평가 시 학생들이 직접 접속할 수 있는 페이지 제공
  - 학생이 자신의 산출물을 직접 업로드할 수 있는 기능 추가
