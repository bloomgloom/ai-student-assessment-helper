...
import { DATA_DIR, ensureDir } from './storage';
...
export function getAssignmentClient(): Client {
  if (!client) {
...
export async function initAssignmentDb(): Promise<void> {
  if (initialized) return;
  const db = getAssignmentClient();
...
async function ensureColumn(table: string, column: string, definition: string): Promise<void> {
  if (!(await columnExists(table, column))) {
    await getAssignmentClient().execute(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`);
  }
}

async function columnExists(table: string, column: string): Promise<boolean> {
  const rows = await getAssignmentClient().execute(`PRAGMA table_info(${table})`);
...
function rowToObject<T>(row: unknown, columns: string[]): T {
  if (Array.isArray(row)) {
    return Object.fromEntries(row.map((value, index) => [columns[index], value])) as T;
  }
  return row as T;
}

export async function assignmentQueryOne<T>(sql: string, args: QueryArgs = []): Promise<T | null> {
  await initAssignmentDb();
  const rs = await getAssignmentClient().execute({ sql, args });
  if (!rs.rows.length) return null;
  return rowToObject<T>(rs.rows[0], rs.columns);
}

export async function assignmentQueryAll<T>(sql: string, args: QueryArgs = []): Promise<T[]> {
  await initAssignmentDb();
  const rs = await getAssignmentClient().execute({ sql, args });
  return rs.rows.map((row) => rowToObject<T>(row, rs.columns));
}

export async function assignmentExecute(sql: string, args: QueryArgs = []) {
  await initAssignmentDb();
  const rs = await getAssignmentClient().execute({ sql, args });
  return { lastInsertRowid: rs.lastInsertRowid ?? 0, rowsAffected: rs.rowsAffected };
}

export async function assignmentTransaction<T>(fn: () => Promise<T>): Promise<T> {
  await initAssignmentDb();
  const db = getAssignmentClient();
