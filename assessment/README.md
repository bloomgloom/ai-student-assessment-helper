...
4. 터미널에서 압축을 푼 프로젝트 폴더의 `assessment` 폴더로 이동합니다.

### 1-3. 컨테이너 실행

`assessment` 폴더에서 아래 명령을 실행합니다.
...
Docker Desktop이 실행 중인지 확인한 뒤 `assessment` 폴더에서 다시 실행하세요.
...
GitHub에서 클론한 경우 `assessment` 폴더에서 아래 명령어로 최신 변경사항을 받을 수 있습니다.
